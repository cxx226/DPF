...
class RandomCropMultiHead(object):
    ...
    def __call__(self, image, label, *args):
        ...
        if left > 0 or right > 0 or top > 0 or bottom > 0:
            ...
            nw,nh = image.size
            label[:,0,0] = (label[:,0,0] * w + left) / nw
            label[:,0,1] = (label[:,0,1] * h + top) / nh
            label[:,1,0] = (label[:,1,0] * w + left) / nw
            label[:,1,1] = (label[:,1,1] * h + top) / nh
        ...
        label[:,0,0] = (label[:,0,0] * w - x1) / tw
        label[:,0,1] = (label[:,0,1] * h - y1) / th
        label[:,1,0] = (label[:,1,0] * w - x1) / tw
        label[:,1,1] = (label[:,1,1] * h - y1) / th 
...
def pointRotate(point,angle):
    angle = angle % 360
    angle = math.radians(angle)
    matrix = np.array([[math.cos(angle) ,math.sin(angle)],
              [-math.sin(angle) ,math.cos(angle)]])
    point_offset = point - np.array([0.5, 0.5])
    point_1 = point_offset[:,0,:]
    point_2 = point_offset[:,1,:]  
    rotated_point_1 = np.matmul(matrix,np.transpose(point_1))
    rotated_point_2 = np.matmul(matrix,np.transpose(point_2))
    point_offset[:,0,:]=np.transpose(rotated_point_1)
    point_offset[:,1,:]=np.transpose(rotated_point_2)
    point = point_offset + np.array([0.5, 0.5])
    return point


def draw_point_(x,y,w, h, img,name=None):
    import cv2
    ps = 2
    point_color = (0,0,255)
    thickness = 4
    cv2.circle(img,(int(x*w), int(y*h)),ps,point_color,thickness)
    if name is not None:
...
class RandomRotateMultiHead(object):
    ...
    def __call__(self, image, label=None, *args):
        ...
        p = max((h, w))
        angle = random.randint(0, self.angle * 2) - self.angle
        label = pointRotate(label,angle)
        #angle=10: array([0.55952553, 0.92858947]) -> array([0.63304499, 0.91174173]) 0.4327033869708973->0.4327033869708972
        image = pad_image('reflection', image, h, h, w, w)
        image = image.rotate(angle, resample=Image.BILINEAR)
        image = image.crop((w, h, w + w, h + h))
        return image, label
...
class RandomHorizontalFlipMultiHead(object):
    ...
    def __call__(self, image, label):
        if random.random() < 0.5:
            ...
            label[:,0,0] = 1 - label[:,0,0] 
            label[:,1,0] = 1 - label[:,1,0] 
